...
def render_pdf_bytes(doc: ReportDocument) -> bytes:
    """Render to PDF and return bytes.

    This is useful for agent/chat interfaces (e.g., Teams) where you want to upload
    a PDF without persisting it to disk.
    """
    ...
    buf = io.BytesIO()
    SimpleDocTemplate(buf, pagesize=letter, title=doc.title).build(story)
    return buf.getvalue()
